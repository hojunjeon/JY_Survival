...
사이클 시작 시 아래 순서로 판단한다:

1. `docs/superpowers/plans/` 에 해당 사이클 계획 파일 존재 여부 확인
2. `docs/phase-status.md` 에 해당 사이클 미완료 태스크(`[ ]`) 존재 여부 확인

| 상태 | 행동 |
|------|------|
| 계획 파일 **없음** | 1~3단계(Superpowers→Gstack→GSD) 실행 → `docs/superpowers/plans/` 저장 → T1부터 코딩 |
| 계획 파일 **있음** + 미완료 태스크 **있음** | 1~3단계 **스킵** → 계획 파일 로드 → 다음 미완료 태스크부터 코딩 직행 |
| 계획 파일 **있음** + 미완료 태스크 **없음** | 사이클 완료 처리 |
