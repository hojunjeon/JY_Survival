...
> 설계 문서: `docs/superpowers/specs/2026-04-20-phase10-fix-cycle-design.md`
> 상태: 수정 사이클 완료 / 추가 사이클 완료
> 출처: Gemini_log.md #8~#9 (플레이 테스트 피드백 3인 페르소나)

### 수정 사이클 태스크 ✅ 완료
- [x] I-frame(피격 무적 시간) 구현 — 적 접촉 시 틱뎀 즉사 버그 해결
- [x] REVERSED CONTROLS 텍스트 위치 이동 — 화면 중앙 → 화면 상단 고정
- [x] 조작 반전 이벤트 발동 타이밍 조정 — 시작 10~15초 → 중반(1~2분) 이후로 이동
- [x] C/C++ 에임 시스템 개선 — 이동 방향 고정 → 마우스 커서 방향 or 최근접 적 자동 에임
- [x] Python 공속·연사력 버프 — 초반 웨이브 대응 불가 문제 해결
- [x] Java 무기 밸런스 재조정 — 근접 리스크 대비 딜 상향 (Phase 9 이월 항목 포함)
- [x] 적(핑크 버그 등) 이동 속도 너프 — 플레이어 기본 이속 상향 or 적 이속 하향

### 추가 사이클 태스크 ✅ 완료
- [x] 타격 이펙트 강화 — ParticleSystem 신규, 피격 플래시(hitFlash), Screen Shake 전면 적용
- [x] 무기별 시각 피드백 개선 — 데미지 텍스트 팝업(크리티컬 포함), Python원형/C마름모 투사체 shape
