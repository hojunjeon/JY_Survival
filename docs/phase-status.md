...
## 현재 Phase: Phase 5 진행 중 (Cycle 2 추가 — 진행 중)
...
- [x] T1: 이벤트 화면 글자 배치 (위아래 중앙 배치) — `bd2d7fb`
- [x] T2: 몬스터 피격 반짝임 범위 수정 (몬스터 크기에 맞게 클리핑)
- [x] T3: 이벤트 화면 픽셀 이미지 상반신만 표시 — drawSpriteClipped 추가
- [x] T4: E3→E2 개명 + 무한 스폰 + 30초 생존 미션으로 교체
- [x] T5: 보스 폭주 2페이즈 붉은 사각형 버그 수정 (drawSpriteTinted 추가)
...
## 현재 Phase: Phase 6 진행 중 (Cycle 2 추가 — T7 완료)
