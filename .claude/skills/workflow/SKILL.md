...
2. 모델 Haiku 전환 (`/model haiku`) — 기계적 구현은 저가 모델로 충분
3. git worktree 생성 (`superpowers:using-git-worktrees`)
4. 독립 task: `Agent` tool로 병렬 dispatch (`run_in_background: true`)
5. 공유 파일 task: 완료 후 순차 dispatch
6. 전체 완료 후 Sonnet 복귀 (`/model sonnet`)
7. Step 3 (Review) 진행
